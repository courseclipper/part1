...
function Catagory() {
  ...
  const OnDeleteCatHandler = async (event, cat) => {
    ...
    try {
      const response = await axios.delete(`http://172.31.47.60:3001/category/delete/${cat.name}`);
  ...
  const AddCategoryHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.get('http://172.31.47.60:3001/category', requestOptions);
  ...
  const PlatformAddHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://172.31.47.60:3001/platform', data_platform);
      if (response.statusText == 'OK') {
  ...
  const PlatformDeleteHandler = async (e, name_plat) => {
    e.preventDefault();
    try {
      const response_del = await axios.delete(`http://172.31.47.60:3001/platform/${name_plat}`);
  ...
  useEffect(() => {
    axios.get("http://172.31.47.60:3001/category").then((response) => {
      setCategory(response.data.Categories);
    }).catch((error) => {
      console.log(error);
    })

    axios.get("http://172.31.47.60:3001/platform").then((response) => {
